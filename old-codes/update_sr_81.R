...
# Specify FILE
FILE <- "20200410-sitrep-81-covid-19.pdf"
...
# less than 1521252 by 6
# Saint Barthelemy, because of blank in transmission classification
df_table2 %>% 
...
setdiff(unique(table2$area), df_table2$area) # Saint Barthelemy

df_table2 <- bind_rows(df_table2,
                       tibble(
                         area = "Saint Barthelemy",
                         cum_conf = 6,
                         new_conf = 0,
                         cum_deaths = 0,
                         new_deaths = 0,
                         publish_date = as.Date("2020-04-10")
                       )
             )
