...
saveRDS(data_usa, "rds/data_usa.rds")
...
# add population
population <- read_excel("data-raw/nst-est2019-01.xlsx", 
...
data_usa <- data_usa %>% 
  left_join(population, by = c("State", "state_name"))

data_usa_ma7_std <- data_usa %>% 
  select(-State) %>% 
  rename(state = state_name) %>% 
  mutate(state = state %>% str_to_lower()) %>% 
  as_tsibble(key = c(state, party, pop), index = publish_date) %>% 
  tq_ma(7) %>% 
  as_tibble() %>% 
  mutate(across(cum_conf:new_deaths, ~ .x * 1000000 / pop))

saveRDS(data_usa_ma7_std, "rds/data_usa_ma7_std.rds")
...
states_map %>% 
  left_join(states_party, by = c("iso_3166_2" = "State")) %>% 
  ggplot() +
...
states_map %>% 
  left_join(states_speed, by = c("iso_3166_2" = "State")) %>% 
  ggplot() +
  geom_sf(aes(fill = increase_in_14_days), color = "white") +  
...
states_map %>% 
  left_join(states_speed_std, by = c("iso_3166_2" = "State")) %>% 
  ggplot() +
  geom_sf(aes(fill = increase_std_in_14_days), color = "white") +  
  scale_fill_gradient2(low = "#559999", mid = "grey90", high = "#BB650B",
...
states_map %>% 
  left_join(states_fatality, by = c("iso_3166_2" = "State")) %>% 
  ggplot() +
  geom_sf(aes(fill = fatality_rate), color = "white") +  
  scale_fill_gradient2(low = "#559999", mid = "grey90", high = "#BB650B",
...
states_map %>% 
  left_join(states_deaths, by = c("iso_3166_2" = "State")) %>% 
  ggplot() +
  geom_sf(aes(fill = deaths_per_1m), color = "white") +  
  scale_fill_gradient2(low = "#559999", mid = "grey90", high = "#BB650B",
