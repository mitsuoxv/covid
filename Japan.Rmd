...
pop <- pop[, c(3, 4, 6)]
names(pop) <- c("code", "prefecture", "population")
...
data_japan <- data_japan %>% 
  left_join(pop %>% select(-prefecture), by = "code")

saveRDS(data_japan, "rds/data_japan.rds")

```


```{r map_draw_prepare, echo=FALSE, message=FALSE}
