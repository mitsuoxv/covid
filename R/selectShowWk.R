...
#' @examples
#' \dontrun{
#' selectShowWkUI("japan_wk", japan_wk$area_df, japan_wk$area_menu)
#' }
selectShowWkUI <- function(id, df, a_menu) {
...
#' Select show weekly module Server
#'
#' @param id A character vector of length 1.
#' @param df A data frame with columns: publish_date, concept, value and var.
#'
...
#' @examples
#' \dontrun{
#' selectShowWkServer("japan_wk", japan_wk$area_df)
#' }
selectShowWkServer <- function(id, df) {
  moduleServer(id, function(input, output, session) {
    ...
    output$download <- downloadHandler(
      filename = function() {
        ...
        paste0(dplyr::if_else(input$perhos == "Yes", "new_conf_per_hospital",
                       "new_conf"),
               ".tsv")
      },
      content = function(file) {
        chart_data() %>%
          dplyr::select(publish_date, {{ area_var }}, !!value_var()) %>%
